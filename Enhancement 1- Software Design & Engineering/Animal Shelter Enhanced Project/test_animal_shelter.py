...
class TestAnimalShelterCRUD(unittest.TestCase):
    ...
    def test_read_with_case_insensitivity(self):
        """Test reading data with case insensitivity"""
        # Here I am testing that read operations are not case-sensitive for the name field.
        criteria = {"name": "test animal"}  # Lowercase criteria
        result = self.shelter.read(criteria)
        self.assertGreaterEqual(len(result), 1, "Read operation failed for case-insensitive criteria")
    ...
    def test_delete_all(self):
        """Test deleting all documents in the collection"""
        # Here I am testing the deletion of all documents in the collection to ensure proper cleanup.
        criteria = {}  # Empty criteria to delete all documents
        deleted_count = self.shelter.delete(criteria)
        self.assertGreaterEqual(deleted_count, 1, "Delete all operation failed")
